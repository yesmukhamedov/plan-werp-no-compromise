...
Числа в этом документе — **плейсхолдеры до измерения текущей системы**. Задавать
целевые показатели, не зная нынешних, бессмысленно: можно поставить недостижимую
планку или, наоборот, узаконить деградацию.

Измерение текущих показателей — работа Фазы 0
([EPIC-009](../../backlog/EPIC-009-baseline-measurement.md)). После неё каждый
показатель получает три значения: **сейчас**, **не хуже чем** (обязательный
минимум) и **цель**.

Правило: новая система **не имеет права быть медленнее старой ни на одном
сценарии**. Переписывание, после которого пользователь работает медленнее, —
провал независимо от качества кода.

## Производительность

| Показатель | Сейчас | Не хуже | Цель |
|---|---|---|---|
...
RPO и RTO подтверждаются владельцем системы — [OQ-006](../08-risks/02-open-questions.md).
...
Отдельная группа требований, вытекающая из [big bang](../02-decisions/ADR-0001-strategy-big-bang.md):
...
| Время отката к легаси | ≤ 1 ч, проверено репетицией |
| Срок стабилизации с работающим легаси в резерве | согласовать; предварительно 30 дней `?` |
...
Вынесено в [06-security.md](06-security.md).

## Наблюдаемость

Вынесено в [06-quality/03-observability.md](../06-quality/03-observability.md).
...
Подробно — [06-quality/02-performance.md](../06-quality/02-performance.md).

...
- Отдельный трек миграции данных → [04-migration/02-data-migration.md](../04-migration/02-data-migration.md),
...
- **Типы для денег.** Правила округления и точность фиксируются один раз
  ([03-target/04-data-model.md](../03-target/04-data-model.md)); поведение
...
- **Пиковая нагрузка.** Профиль нагрузки снимается с текущей системы до
  проектирования схемы, а не после → [06-quality/02-performance.md](../06-quality/02-performance.md).

### Открытые вопросы

- Версия PostgreSQL, топология (реплики, отказоустойчивость), способ
  резервного копирования и целевые RPO/RTO — [OQ-006](../08-risks/02-open-questions.md).
- Есть ли в Oracle хранимая бизнес-логика (пакеты, триггеры, задания планировщика
  БД), не видимая из кода приложения — [OQ-007](../08-risks/02-open-questions.md).
